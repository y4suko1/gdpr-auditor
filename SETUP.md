...
2. In the Project's **Set project instructions** field, paste the instruction block from the main README. This isn't strictly required — the connected files already act as instructions once loaded as knowledge — but the instructions field is read on every chat in the Project, so it's a reliable backstop against a vague prompt skipping the full procedure.
...
- `examples.md` — five worked findings showing the required format, each with a real file:line citation
