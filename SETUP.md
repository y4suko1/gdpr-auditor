...
**Clone-and-verify, and why it's pinned to a tag.** If the Project has code execution and github.com access enabled, `rules.md` section 7 has the auditor clone the repository itself before delivering a report, so citations are checked against the actual cloned files with `verify/check.py`, not just search results. That clone is pinned to a release tag (`v1.1.0`), not `main`. `main` moves every time this repo is edited, including mid-edit; a tag is frozen, so every audit run against it gets the same reference text and rules until a new tag is deliberately cut. When you make a change worth shipping, cut a new tag and update the version number in the README's instructions block and here to match, don't leave the instructions pointing at a stale tag or, worse, at `main`.
